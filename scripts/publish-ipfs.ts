...
async function pinDirectoryWithPinata(distDir: string): Promise<string> {
  ...
  for (const entry of fileEntries) {
    ...
    // Convert Windows backslashes to forward slashes for Pinata
    const normalizedPath = entry.relPath.replace(/\\/g, '/');
    files.push(new File([buffer], normalizedPath, { type: guessMimeType(entry.absPath) }));
  }

  const upload: UploadResponse = await pinata.upload.public.fileArray(files).name('shashankg-site');
